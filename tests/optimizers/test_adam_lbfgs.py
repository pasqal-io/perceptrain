...
def test_adam_lbfgs_initialization(Basic: nn.Module) -> None:
...
def test_adam_lbfgs_state_dict(adamlbfgs_optimizer: AdamLBFGS) -> None:
...
def test_adam_lbfgs_load_state_dict(Basic: nn.Module) -> None:
...
def test_adam_phase_step(Basic: nn.Module, adamlbfgs_optimizer: AdamLBFGS) -> None:
    x = torch.randn(10, 1)
    y = torch.randn(10, 1)
...
def test_lbfgs_phase_step(Basic: nn.Module) -> None:
...
def test_lbfgs_requires_closure(Basic: nn.Module) -> None:
...
def test_custom_parameters(Basic: nn.Module) -> None:
    adam_kwargs = {"lr": 0.001, "betas": (0.8, 0.999)}
    lbfgs_kwargs = {"lr": 0.1, "max_iter": 10}

    opt = AdamLBFGS(
        Basic.parameters(), switch_epoch=3, adam_kwargs=adam_kwargs, lbfgs_kwargs=lbfgs_kwargs
    )
