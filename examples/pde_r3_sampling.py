...
from perceptrain.data import DictDataLoader, InfiniteTensorDataset
...
class R3Sampling(Callback):
    def __init__(
        self,
        initial_dataset: TensorDataset | InfiniteTensorDataset,
        label_function: Callable[[torch.Tensor], torch.Tensor],
        fitness_function: Loss = MSELoss(),
        threshold: float = 0.1,
        sampling_distribution: Callable[[int], torch.Tensor] | None = None,
        ...
        self.features = initial_dataset.tensors[0]  # NOTE constraint on the dataset
        self.labels = initial_dataset.tensors[1]  # NOTE constraint on the dataset
        self.label_function = label_function

        self.n_samples_total = len(self.features)
    ...
    def run_callback(self, trainer: Any, config: TrainConfig, writer: BaseWriter) -> None:
        ...
        fitnesses, _ = self.fitness_function(trainer.model, (self.features, self.labels))
        ...
        # Release
        self.features = torch.where(retained, self.features, new_features)

        # Compute labels
        self.labels = self.label_function(self.features)

        # Re-init dataset
        self.dataset.__init__(self.features, self.labels)

        # Update dataloader of the trainer
