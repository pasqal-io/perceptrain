In `qadence`, the [`QNN`][qadence.models.QNN] is a variational quantum model that can potentially take multi-dimensional input.
...
[`ObservableConfig`][qadence.constructors.hamiltonians.ObservableConfig], [`FeatureMapConfig`][qadence.config.FeatureMapConfig], [`AnsatzConfig`][qadence.config.AnsatzConfig]. These classes allow you to specify the type of circuit and the parameters of the circuit in a structured way.
...
For full details on the `FeatureMapConfig` class, see the [API documentation][qadence.config.FeatureMapConfig].
